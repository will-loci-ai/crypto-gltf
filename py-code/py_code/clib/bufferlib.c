...
void extend(Buffer *buffer, uint32_t block)
{
    ...
    {
        // fill in rest of idx and add remaining block to next idx
        assertmsg(buffer->idx < buffer->size, "Error: end of buffer reached.");
        buffer->arr[buffer->idx] |= block >> (buffer->offset + buffer->len - 32);
        buffer->idx++;

        assertmsg(buffer->arr[buffer->idx] == 0, "Error: non-zero buffer provided.");
        buffer->arr[buffer->idx] |= block << (32 - buffer->offset - buffer->len);
    ...
    else
    {
        ...
        if (buffer->offset == 32)
        {
            buffer->offset = 0;
            buffer->idx++;
            if (buffer->idx != buffer->size) // i.e. end of buffer not yet reached
            {
                assertmsg(buffer->arr[buffer->idx] == 0, "Error: non-zero buffer provided.");
            }
        }
...
void fill_buffer(Buffer *buffer, uint32_t fill)
{
    ...
    // important for encryption padding so padded values are random bits
    if (buffer->idx == buffer->size)
    {
        // final buffer element already filled, buffer->len is divisible by 32
