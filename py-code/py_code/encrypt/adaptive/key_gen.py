...
from py_code.encrypt.adaptive.utils import combine_sblocks, get_sblocks
...
def generate_keys(
    plnm: PlnM, params: AdaptiveCipherParams, k3: bytes | None = None
) -> Key:
...
def get_k2(
    float_arrs: list[np.ndarray], params: AdaptiveCipherParams, k3: bytes
) -> bytes:
...
def get_k1(
    float_arrs: list[np.ndarray], params: AdaptiveCipherParams, k2: bytes
) -> bytes:
